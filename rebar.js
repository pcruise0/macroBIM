...
class RebarBase {
    constructor(center, dims, rotation = 0) { 
        ...
        this.id = "REBAR_" + Math.floor(Math.random() * 1000);
    }

    makeSeg(p1, p2, normal, initialState) {
        // 물리 엔진용 노드 생성 (간격 40%, 60%)
        let nodes = []; 
        const ratios = [0.4, 0.6];
        ratios.forEach(ratio => { 
            nodes.push({ x: p1.x + (p2.x - p1.x) * ratio, y: p1.y + (p2.y - p1.y) * ratio, vx: 0, vy: 0 }); 
        });
        let dx = p2.x - p1.x; 
        let dy = p2.y - p1.y; 
        let initialLen = Math.sqrt(dx * dx + dy * dy);
        return { 
...
class Shape11 extends RebarBase {
    generate() {
        ...
        // 법선 설정: 바깥쪽 벽면을 향하도록 (Cantilever 구석 안착용)
        this.segments = [ 
...
class Shape23 extends RebarBase {
    generate() {
        ...
        let p1 = { x: x, y: y + A }; let p2 = { x: x, y: y }; 
        let p3 = { x: x + B, y: y }; let p4 = { x: x + B, y: y - C };
        this.segments = [ 
            this.makeSeg(p1, p2, {x:-1, y:0}, "FITTING"), 
            this.makeSeg(p2, p3, {x:0, y:-1}, "WAITING"), 
...
class Shape25 extends RebarBase {
    generate() {
        ...
        let ax = Math.sqrt(Math.max(0, A*A - C*C)); 
        let bx = Math.sqrt(Math.max(0, B*B - D*D));
        let p1 = { x: x - E/2 - ax, y: y + C };
        let p2 = { x: x - E/2,      y: y };
        let p3 = { x: x + E/2,      y: y };
        let p4 = { x: x + E/2 + bx, y: y + D };
        this.segments = [ 
            this.makeSeg(p1, p2, {x:-1, y:0.5}, "FITTING"), 
            this.makeSeg(p2, p3, {x:0, y:-1}, "WAITING"), 
            this.makeSeg(p3, p4, {x:1, y:0.5}, "WAITING") 
        ];
        this.applyRotation();
        return this;
    }
...
class Shape44 extends RebarBase {
    ...
    finalize() {
        // A, E 날개의 각도를 물리 엔진이 찾은 벽면 각도로 업데이트
        let angA = Math.atan2(this.segments[0].nodes[1].y - this.segments[0].nodes[0].y, this.segments[0].nodes[1].x - this.segments[0].nodes[0].x);
        let angE = Math.atan2(this.segments[4].nodes[1].y - this.segments[4].nodes[0].y, this.segments[4].nodes[1].x - this.segments[4].nodes[0].x);
        
        super.finalize(); // 내부 코너 교점 정리

        // 길이 고정 및 방향 정렬
        let sA = this.segments[0];
        sA.p1 = { x: sA.p2.x - Math.cos(angA) * sA.initialLen, y: sA.p2.y - Math.sin(angA) * sA.initialLen };
        let sE = this.segments[4];
        sE.p2 = { x: sE.p1.x + Math.cos(angE) * sE.initialLen, y: sE.p1.y + Math.sin(angE) * sE.initialLen };
    }
}
...
class Shape63 extends RebarBase {
    generate() {
        ...
        this.segments = [ 
            ...
            this.makeSeg(p2, p3, {x:0, y:-1}, "WAITING"), 
            this.makeSeg(p3, p4, {x:1, y:0}, "WAITING"), 
            this.makeSeg(p4, p1, {x:0, y:1}, "WAITING") 
        ];
        this.applyRotation();
        return this;
    }
    finalize() {
        super.finalize();
        // 마지막 마디와 첫 마디 교차 처리 (폐합)
        let last = this.segments[3]; let first = this.segments[0];
...
class RebarFactory { 
    static create(code, center, dims, rotation = 0) { 
        let r = null; let c = parseInt(code);
        if(c===1) r=new Shape01(center,dims,rotation);
        else if(c===11) r=new Shape11(center,dims,rotation);
        else if(c===23) r=new Shape23(center,dims,rotation);
        else if(c===25) r=new Shape25(center,dims,rotation);
        else if(c===44) r=new Shape44(center,dims,rotation);
        else if(c===63) r=new Shape63(center,dims,rotation);
